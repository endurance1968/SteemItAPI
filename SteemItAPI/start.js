...
function evalAccountHistoryPayments(accountname, transaction, trxindex){
    ...
    if (operation[0] == 'author_reward') {
        ...
        mongoapi.insertObject(dbo, "steem_trx_" + operation[0], transaction);
    ...
    mongoapi.updateOne(dbo, config.mongo.steem_account_collection, { account: accountname }, { $set: { last_ptrx: trxindex } },updateOneCB)
    return true;
}
...
//
function getAccountHistoryCB(account, from, err, result = []) {
    if (err == null) {
        // we should get an array with data of accounts
        if (result.length > 0) {
            ...
            if (newfrom > config.steem.accounthistmaxretr) {
                console.log("getAccountHistory from index " + (newfrom + config.steem.accounthistmaxretr + 1) + " for " + account);
                steemapi.getAccountHistory(account, newfrom, config.steem.accounthistmaxretr, getAccountHistoryCB);
            }
            else {
                if (newfrom > 0) {
                    console.log("getAccountHistory get the last " + newfrom + " history entries for " + account);
                    steemapi.getAccountHistory(account, newfrom, newfrom, getAccountHistoryCB);
                }
                else
                    console.log("getAccountHistory for " + account + " done");
            }
        }
        else
        ...
        for (let i = (result.length-1); i >=0 ; i--) {
            ...
            var handled = evalAccountHistoryPayments(account, transaction,result[i][0]);
            if (handled == false) {
                //console.log("   transaction-block: " + transaction.block);
                let operation = transaction.op;
                //console.log("   transaction-id: " + transaction.trx_id + " timestamp: " + transaction.timestamp+" operationtype: " + operation[0]);
        ...
        //err!=null
        console.log("getAccountHistory failed "+err);
    }
}
...
function createIndexCB(err,indexName) {
    console.log("ensureIndexCB called index: " + indexName);
    ...
    //steemapi.getAccountHistory("endurance1968", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("taxguy", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    //steemapi.getAccountHistory("steem", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);

    /*
    steemapi.getAccountHistory("endurance1968", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("theaustrianguy", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("taxguy", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("sempervideo", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("lenatramper", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("uwelang", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("steemitblog", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("berniesanders", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("adsactly", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("haejin", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("jedigeiss", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("dtube", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    steemapi.getAccountHistory("steem", -1, config.steem.accounthistmaxretr, getAccountHistoryCB);
    */
}
...
function runCB() {
    ...
    mongoapi.createUniqueIndex(dbo, config.mongo.steem_account_collection, "account", createIndexCB);
